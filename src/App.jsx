...
const ProductCard = ({ item, onAddToCart, isAdded }) => (
    <div className="group rounded-3xl border border-slate-200 bg-white p-5 shadow-sm transition duration-500 hover:-translate-y-1 hover:shadow-lg">
        <div className="h-48 rounded-3xl bg-gradient-to-br from-slate-100 via-slate-200 to-slate-100 transition duration-500 group-hover:scale-105" />
...
const App = () => {
    ...
    const handleAddToCart = (item) => {
        setCartItems((prev) => {
            if (prev.some((entry) => entry.name === item.name)) return prev
            return [...prev, item]
        })
    ...
    return (
        <div className="min-h-screen bg-slate-100 text-slate-900">
            <Navbar />
            {toastMessage && (
                <div className="fixed bottom-28 right-6 z-20 rounded-3xl bg-emerald-600 px-5 py-3 text-sm font-semibold text-white shadow-2xl transition-opacity duration-300">
                    {toastMessage}
                </div>
            )}
            <div className="fixed bottom-6 right-6 z-20 w-[240px] rounded-3xl bg-slate-950/95 px-5 py-4 text-white shadow-2xl backdrop-blur-xl transition duration-300 hover:scale-105 sm:w-auto">
                <p className="text-xs uppercase tracking-[0.3em] text-slate-400">Shopping bag</p>
                <p className="mt-2 text-2xl font-bold">{cartCount} item{cartCount !== 1 ? 's' : ''}</p>
                <button className="mt-4 w-full rounded-full bg-yellow-400 px-4 py-2 text-sm font-semibold text-slate-900 transition hover:bg-yellow-300">
                    View cart
                </button>
            </div>
            <main className="mx-auto max-w-[1280px] px-4 pb-16 pt-6">
                <section className="grid gap-6 rounded-[2rem] bg-gradient-to-r from-slate-900 via-slate-800 to-slate-950 px-6 py-10 text-white shadow-2xl sm:grid-cols-[1.25fr_0.9fr] sm:gap-8">
                    <div className="space-y-6">
                        <span className="inline-flex items-center rounded-full bg-yellow-400 px-3 py-1 text-sm font-semibold text-slate-900 animate-pulse">Prime delivery</span>
                        ...
                        <div className="flex flex-wrap gap-3">
                            <button className="rounded-full bg-yellow-400 px-6 py-3 font-semibold text-slate-900 transition hover:bg-yellow-300 hover:scale-105 animate-pulse">Shop Deals</button>
                ...
                <section className="mt-10">
                    ...
                    <div className="grid gap-4 lg:grid-cols-3">
                        {deals.map((item) => (
                            <div key={item.name} className="rounded-[1.75rem] border border-slate-200 bg-white p-6 shadow-sm transition duration-500 hover:-translate-y-1 hover:shadow-lg">
                                ...
                                <div className="mt-6 flex items-center justify-between">
                                    ...
                                    <button
                                        ...
                                        className="rounded-full bg-slate-900 px-4 py-2 text-sm font-semibold text-white transition hover:bg-slate-700 disabled:cursor-not-allowed disabled:bg-slate-400"
                                        disabled={isAddedToCart(item.name)}
                                    >
                                        {isAddedToCart(item.name) ? 'Added' : 'Buy Now'}
                                    </button>
                                </div>
                            </div>
                        ))}
                    ...
                    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
                        {recommended.map((item) => (
                            <ProductCard
                                ...
                                isAdded={isAddedToCart(item.name)}
                            />
                        ))}
                    </div>
                </section>
